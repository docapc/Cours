...
\defi{Restfull}{Representationel State Transfert  : style d'architechture. En 2000 en parallèle du https 1.1. Framework c\# : Asp.Net.}\\

\rem{Les principes REST}{\begin{itemize} 
\item Séparation client/serveur : on sépare les responsabilité et on communique via l'API car elle est standardisée. Cela permet de la modularité de chaque coté (on peut faire des modifications de chaques cotés sans que cela impact l'autre.)
\item Absence d'état de session (stateless) : l'état d'une session est inclus dans une requete. Les requetes sont traités de manière isolée. Le serveur n'a pas a mémoriser les informations entre les requêtes (performance, et permet de mélanger de gros voluymes de données. L'API ne connait pas les applications avec lesquelles elle discute.)
...
\defi{UFI}{Uniform Resource Identifier. url + resource (cf shéma slide 16) $\to$ la ressource est la requête.}\\
...
\rem{Dans le Program.cs}{Les \input{Service} servent à configurer l'API}
