...
public class UserFakeRepository
{
    ...
    public IList<UserSqlDto> GetPredicate(Func<UserSqlDto, bool> predicate) // Func<type entrée, type sortie>
    {
    ...
    {
        // Users est la liste/dico/collection, Where est la méthode Linq, user est l'objet en cours, Birthday.Year == 1990 est la condition
    ...
    public TimeSpan GetOlderAge()
    {
        ...
        //return (from user in Users select now - user.Birthday).Max(); //.Max() sans paramètres retourne l'élément le plus grand dans la liste
    }
}